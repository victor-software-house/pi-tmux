...
describe("getFlags()", () => {
	test("canAttach is false when autoAttach is 'never'", () => {
		const flags = getFlags({ autoAttach: "never", defaultLayout: "split-vertical", allowMute: true, maxWindows: 10 });
		expect(flags.canAttach).toBe(false);
	});

	test("canAttach is true when autoAttach is 'session-create'", () => {
		const flags = getFlags({ autoAttach: "session-create", defaultLayout: "split-vertical", allowMute: true, maxWindows: 10 });
		expect(flags.canAttach).toBe(true);
	});

	test("canAttach is true when autoAttach is 'always'", () => {
		const flags = getFlags({ autoAttach: "always", defaultLayout: "tab", allowMute: false, maxWindows: 5 });
		expect(flags.canAttach).toBe(true);
	});

	test("canMute mirrors allowMute", () => {
		expect(getFlags({ autoAttach: "never", defaultLayout: "split-vertical", allowMute: true, maxWindows: 10 }).canMute).toBe(true);
		expect(getFlags({ autoAttach: "never", defaultLayout: "split-vertical", allowMute: false, maxWindows: 10 }).canMute).toBe(false);
	});
});
